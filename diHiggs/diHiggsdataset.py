...
class diHiggsDataset(InMemoryDataset):
     ...
     # Take Donatella's code but in process() have instead
     ...
     def process(self):
         ...
         for event_type, h5_file in h5_files.items():
                 ...
                 #vectorized version of convert()
                 convert_vec = np.vectorize(convert)
                 #If a cell contains a dictionary with a single key-value pair, it replaces it with either:
                 #The first element of the value (if it's a list)
                 #The value itself (if not a list).
